...
;Console Functions + More... ;{
	...
	Getch(ByRef keyname="") {
		...
		If (A_IsUnicode) { ;enable unicode "getch" => _getchW()
			key:=_getchw()
			if (key==224) or (key==0) ; '�' in Unicode is 224 or 0xE0
			{
		...
		else if (key==224) ;//or FFFFFFE0 or 4294967264 ;*keyname ="Special";
		{
			...
			{
				keyname:=chr(224) ; '�' in Unicode is 224 or 0xE0
				return 224
			}
			else
				keyname:="Special"
	...
	*/
	...
	ReadConsoleOutput(x, y, w, h, tot=0) {
		str:=""
		Loop % h
		{
			if(tot)
			...
			{
				k:=(A_Index-1)+x
				line .= ReadConsoleOutputCharacter(k,z)
			}
			if(tot)
				str:=line . str
			else
